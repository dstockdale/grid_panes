defmodule GridPanesWeb.GridLive.GridsFormComponent do
  ...
  def render(assigns) do
      ...
      >
        ...
        <div class="mt-6">
          ...
          <div class="flex gap-2 mb-4">
            <.button type="button" phx-click="add-root-pane" phx-target={@myself} class="text-sm">
              + Add Root Pane
            </.button>
            <.button type="button" phx-click="add-group" phx-target={@myself} class="text-sm ml-2">
              + Add Group
            </.button>
          </div>
    ...
    <!-- Detailed pane forms -->
        ...
        <footer class="mt-6 flex gap-2">
          ...
          <.button type="button" phx-click="cancel" phx-target={@myself} class="ml-2">Cancel</.button>
        </footer>
      </.form>
    </div>
    """
  ...
  def handle_event("remove-pane", %{"pane_index" => index}, socket) do
    ...
    panes = get_panes_from_changeset(form)

    if index < length(panes) do
      pane_to_remove = Enum.at(panes, index)
      updated_panes = remove_pane_and_update_references(panes, pane_to_remove)
  ...
  defp add_pane(socket, attrs) do
    ...
    updated_panes = add_pane_and_update_parent(panes ++ [new_pane], new_pane)
    changeset = Ecto.Changeset.put_embed(form, :panes, updated_panes)
  ...
  defp add_pane_and_update_parent(panes, new_pane) do
    case new_pane.parent_id do
      nil ->
        panes

      parent_id ->
        Enum.map(panes, fn pane ->
          if pane.id == parent_id do
            %{pane | children: pane.children ++ [new_pane.id]}
          else
            pane
          end
        end)
  ...
  defp remove_pane_and_update_references(panes, pane_to_remove) do
    panes
    |> Enum.reject(&(&1.id == pane_to_remove.id))
    |> Enum.map(fn pane ->
      # Remove from children lists
      updated_children = Enum.reject(pane.children, &(&1 == pane_to_remove.id))
      %{pane | children: updated_children}
    end)
    |> Enum.map(fn pane ->
      # Update parent_id if this pane was a child of the removed pane
      if pane.parent_id == pane_to_remove.id do
        %{pane | parent_id: pane_to_remove.parent_id}
      else
        pane
      end
    end)
  ...
  defp get_pane_tree(form) do
    panes = get_panes_from_changeset(form.source)
    build_tree(panes)
  end

  defp get_all_panes(form) do
    get_panes_from_changeset(form.source)
  end

  defp get_panes_from_changeset(changeset) do
    case Ecto.Changeset.get_embed(changeset, :panes) do
      nil ->
        []

      panes ->
        # Convert changesets to structs if needed
        Enum.map(panes, fn
    ...
    <div style={"margin-left: #{@level * 1}rem;"}>
      <div class="flex items-center gap-2 p-2 bg-white border rounded group hover:bg-gray-50">
        <div class="flex items-center gap-2 flex-1">
          <span class={["text-xs px-2 py-1 rounded", pane_type_classes(@node.pane.type)]}>
            {@node.pane.type}
          </span>
  ...
  defp pane_type_classes(:group), do: "bg-blue-100 text-blue-800"
  defp pane_type_classes(:pane), do: "bg-green-100 text-green-800"
  defp pane_type_classes(_), do: "bg-gray-100 text-gray-800"
    ...
    <div class="border border-gray-200 rounded-lg p-4 mb-4">
      <div class="flex items-center justify-between mb-4">
        <h4 class="font-medium">
          ...
          <span class={["ml-2 text-xs px-2 py-1 rounded", pane_type_classes(@form[:type].value)]}>
            {@form[:type].value}
          </span>
        </h4>
      </div>
      ...
      <div :if={@form[:type].value in [:pane, :group]} class="mt-4">
        ...
        <div class="grid grid-cols-3 gap-4">
          <.input
            ...
            type="select"
            label="Unit"
            options={Pane.size_unit_options()}
          />
          <.input
      ...
      <div class="mt-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <.input
            field={@form[:divider_position]}
            type="select"
            label="Position"
            options={Pane.divider_position_options()}
          />
          <.input
